...
<div class="p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <form class="space-y-4" id="createContent" enctype="multipart/form-data">
            <div>
                {{-- error messgae --}}
                ...
                <div class="mb-4">
                    <label for="grade" class="block text-gray-700 font-bold mb-2">ระดับชั้น</label>
                    <select name="grade" id="grade" class="w-full border-gray-300 rounded">
                        @foreach ($levels as $level)
                            <option value="{{ $level->id }}" {{ old('grade') == $level->id ? 'selected' : '' }}>
                                {{ $level->name }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="mt-6 text-center">
        <x-primary-button id="nextStep" type="button"
            class="w-full md:w-auto px-8 py-3 bg-blue-500 text-white rounded-lg gap-1">ดำเนินการต่อ <svg
